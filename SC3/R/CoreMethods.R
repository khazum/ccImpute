...
sc3_prepare.SingleCellExperiment <- function(object, gene_filter, pct_dropout_min, pct_dropout_max, 
  ...
  if (length(n_dim) > 15) {
    upper_bound<-ceiling(d_region_max * ncol(object))
    lower_bound<-floor(d_region_min * ncol(object))
...
#' Can also be a single integer.
#' @param clust_method - if True uses fast community clustering, otherwise uses k means.
#' 
#' @return an object of \code{SingleCellExperiment} class
#' 
#' @importFrom doRNG %dorng%
...
sc3_kmeans.SingleCellExperiment <- function(object, ks, clust_method) {
